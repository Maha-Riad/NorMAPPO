...
import numpy as np
import itertools #added by Maha
...
class PPO(nn.Module):
    def __init__(self, state_shape, in_channels=6, n_actions=9):
        ...
        #added by Maha
        self.joint_action=True
        number_of_joint_actions=3 # Change this to 2 in case of two agents only learning
        if self.joint_action==True:
            self.joint_actions_list=self.createJointAction(n_actions, number_of_joint_actions)
            n_actions=len(self.joint_actions_list)
    ...
    def act(self, state):
        ...
        if self.joint_action==True:
            x_actor_joinAction=[]
            for a in action:
                x_actor_joinAction.append(self.joint_actions_list[a.item()])
            return np.array(x_actor_joinAction), m.log_prob(action).detach().cpu().numpy()
           #end of Maha's addition
    ...
    def createJointAction(self, n_actions=9, number_of_joint_actions=2): #added by Maha
        actions=[]
        for action in range (1,n_actions+1):
            actions.append(action)
        actions=itertools.product(*([actions] * number_of_joint_actions))
        j_actions = []
        for action in actions:
            j_actions.append(list(action))
        return j_actions
    def revertJointActions(self, actions):
        ### convert back the actions here
        actionsIndex=[]
        for a in actions.tolist():
            actionsIndex.append(self.joint_actions_list.index(a))
        return torch.Tensor(actionsIndex).type(torch.int32)
