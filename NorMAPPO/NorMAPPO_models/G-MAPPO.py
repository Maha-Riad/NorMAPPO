...
if __name__ == '__main__':
    ...
    for t in tqdm(range(int(config.env_steps / config.n_workers))):
        ...
        train_ready1 = dataset1.write_tuple(states, action1, scalarized_rewards, done, log_probs1, rewards)
        train_ready2 = dataset2.write_tuple(states, action2, scalarized_rewards, done, log_probs2, rewards)
        train_ready3 = dataset3.write_tuple(states, action3, scalarized_rewards, done, log_probs3, rewards)
        if train_ready1:
            ...
            for i in range(objective_logs1.shape[1]):
                ...
                print ('The objectives mean of agent1 is: ', objective_logs1[:, i].mean())#added by Maha for tracing
            for ret in dataset1.log_returns():
                wandb.log({'agent1_Returns': ret})
                print ('The return of agent1 is' , ret)#added by Maha for tracing
        ...
        if train_ready2:
            ...
            for i in range(objective_logs2.shape[1]):
                ...
                print ('The objectives mean of agent2 is: ', objective_logs2[:, i].mean())#added by Maha for tracing
            for ret in dataset2.log_returns():
                wandb.log({'agent2_Returns': ret})
                print ('The return of agent2 is' , ret)#added by Maha for tracing
        ...
        if train_ready3:
            ...
            for i in range(objective_logs3.shape[1]):
                ...
                print ('The objectives mean of agent3 is: ', objective_logs3[:, i].mean())#added by Maha for tracing
            for ret in dataset3.log_returns():
                wandb.log({'agent3_Returns': ret})
                print ('The return of agent3 is' , ret)#added by Maha for tracing
    ...
    #vec_env.close()
    torch.save(ppo1.state_dict(), 'saved_models/Agent1_3PPO_3Datasets_1Reward_' + str(config.lambd) + '.pt')
    torch.save(ppo2.state_dict(), 'saved_models/Agent2_3PPO_3Datasets_1Reward_' + str(config.lambd) + '.pt')
    torch.save(ppo3.state_dict(), 'saved_models/Agent3_3PPO_3Datasets_1Reward_' + str(config.lambd) + '.pt')
